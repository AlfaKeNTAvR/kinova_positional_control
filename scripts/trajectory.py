...
class KinovaTrajectory:
    ...
    def __init__(
        ...
        self.__MAX_LINEAR_SPEED = max_linear_speed  # [meters/s].
        self.__MAX_ANGULAR_SPEED = max_angular_speed  # [deg/s].
        self.__MIN_LOOP_FREQUENCY = 100
        ...
        # # Service provider:
        rospy.Service(
            f'{self.__NODE_NAME}/upload_trajectory',
            UploadTrajectory,
            self.__upload_trajectory_handler,
        )
        rospy.Service(
            f'{self.__NODE_NAME}/resume_trajectory',
            Trigger,
            self.__resume_trajectory_handler,
        )
        rospy.Service(
            f'{self.__NODE_NAME}/pause_trajectory',
            Trigger,
            self.__pause_trajectory_handler,
        )
        rospy.Service(
            f'{self.__NODE_NAME}/cancel_trajectory',
            Trigger,
        ...
        self.__kinova_pose = rospy.Publisher(
            f'/{self.__ROBOT_NAME}/positional_control/input_pose',
            Pose,
            queue_size=1,
        )
        self.__trajectory_finished = rospy.Publisher(
            f'{self.__NODE_NAME}/trajectory_finished',
            Bool,
            queue_size=1,
        )
        ...
        # # Topic subscriber:
        rospy.Subscriber(
            f'/{self.__ROBOT_NAME}/positional_control/kinova_relaxed_ik_missalignment',
            Pose,
...
def main():
    ...
    max_linear_speed = rospy.get_param(
        param_name=f'{node_name}/max_linear_speed',
        default=0.2,
    )

    max_angular_speed = rospy.get_param(
        param_name=f'{node_name}/max_angular_speed',
        default=90,
    )
